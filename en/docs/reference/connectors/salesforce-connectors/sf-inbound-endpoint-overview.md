...
<table>
  ...
  <tr>
    ...
    <td>The authentication method to use when connecting to Salesforce. <code>username-token</code> uses SOAP login with a username, password and token. <code>oauth</code> uses the OAuth2 Client Credentials grant type. Configurations without this parameter default to <code>username-token</code>. OAuth2 Client Credentials support is available from inbound version 3.0.5 and 2.1.17 onwards.</td>
    <td>No</td>
