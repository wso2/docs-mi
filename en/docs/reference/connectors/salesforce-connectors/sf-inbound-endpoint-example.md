...
You can also consume the Change Data Capture (CDC) events for Salesforce Records by enabling it in the Salesforce platform. For more information, see the [Change Data Capture Events](https://developer.salesforce.com/docs/atlas.en-us.change_data_capture.meta/change_data_capture/cdc_select_objects.htm) documentation. Once you enable the CDC events, you can consume the CDC events using the Salesforce Inbound Endpoint by providing the `connection.salesforce.salesforceObject` parameter as `/data/<RecordName>ChangeEvent` (Eg: `/data/AccountChangeEvent`).
