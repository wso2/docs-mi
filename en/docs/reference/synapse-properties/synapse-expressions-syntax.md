...
!!! Note
    We have a dedicated [sample]({{base_path}}/learn/samples/synapse-expressions/) in the MI for VS Code extension to demonstrate each of the following sections. You can deploy the sample in WSO2 Integrator: MI and follow along by invoking the relevant resources to see the results.
