...
!!! Tip
    You can specify JDBC driver- or database vendor-specific settings as URL parameters/properties in the `store.jdbc.connection.url` (the exact syntax is driver-specific). This includes network-related settings, such as connection and socket timeouts, which may need to be fine-tuned based on your environment. For the supported parameters and recommended values, see the documentation for your JDBC driver or database vendor.
