...
Addition `+`, Subtraction `-`, Multiplication `*`, Division `/`, Modulus `%` operators are supported between operands.
...
The following keywords are reserved in Synapse Expressions and cannot be used in value access expressions with dot notation (instead we should use the bracket notation).
...
SynapsePath provides a fault tolerance design which avoids disruptive failures. In case of an error in a complex expression it fails in the first evaluation without propagating the invalid result to rest of the evaluation steps
