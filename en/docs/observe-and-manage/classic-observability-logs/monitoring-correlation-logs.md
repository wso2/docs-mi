...
Product observability enables rapid debugging of product issues. The WSO2 Integrator: MI enables observability using Correlation logs. Correlation logs allow you to monitor individual HTTP requests from the point that a message is received by the WSO2 Integrator: MI until the corresponding response message is sent back to the original message sender. That is, the complete round trip of an HTTP message (client →WSO2 Integrator: MI → back-end →WSO2 Integrator: MI → client) can be tracked and analyzed using a log
file.

When Correlation logs are enabled for the WSO2 Integrator: MI server, a separate log file named `correlation.log` is created in the `<MI_HOME>/repository/logs/` directory. Every HTTP message that flows between the WSO2 Integrator: MI and external clients goes through several state transitions. A new log entry is created in the `correlation.log` file corresponding to the state changes in the round trip of a single HTTP request. A Correlation ID assigned to the incoming HTTP request is assigned to all the log entries corresponding to the request. Therefore, you can use this Correlation ID to easily locate the logs relevant to the round trip of a specific HTTP request and, thereby, analyze the behavior of the message flow.
...
Now when you start the WSO2 Integrator: MI, the `correlation.log` file is created in the `<MI_HOME>/repository/logs/` directory.
...
!!! Tip
    You can customize the behavior of correlation logging, including the name of the HTTP header used for the Correlation ID, the log file location, and other related settings. For full configuration details, see [Configure Correlation Logs]({{base_path}}/observe-and-manage/classic-observability-logs/configuring-log4j2-properties/#correlations-logs).

## Send an HTTP request with a Correlation ID

When a client sends an HTTP request to the WSO2 Integrator: MI, a Correlation ID can be included using the configured correlation header. By default, this header is set to `activityid`.

!!! note
    If the client does **not** include a Correlation ID in the request, the WSO2 Integrator: MI will automatically generate one and assign it to the request.

The Correlation ID, whether provided by the client or generated by the WSO2 Integrator: MI, is added to all log entries related to the same request. This helps you trace the complete flow of the request across the system.

To change the default correlation header, see [Configure Correlation Logs]({{base_path}}/observe-and-manage/classic-observability-logs/configuring-log4j2-properties/#correlations-logs).

The following is a sample POST request sent using the curl client. Note that the Correlation ID is included in the request header.
...
## Read Correlation Logs

The format of a correlation log entry is shown below, followed by an example.
...
The details recorded in each log entry are explained below.
...
<td><div class="content-wrapper">
<p>Each log contains a Correlation ID, which is unique to the HTTP request. A client can send the Correlation ID in the header of the HTTP request. If this Correlation ID is missing in the incoming request, the WSO2 Integrator: MI will generate one for the request.</p>
<p>The name of the HTTP header used to carry the Correlation ID is configurable in the WSO2 Integrator: MI.</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<li>For all other log entries, the duration indicates the time gap between the current log entry and the immediately preceding one. This represents the time taken for the HTTP request to transition from one state to another.</li>
</ul>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
...
<ul>
<li><strong>HTTP</strong> call type identifies logs that correspond to either back-end latency or round-trip latency states. That is, in the case of an individual request, one log will be recorded to identify back-end latency, and another log for round-trip latency. Since these logs relate to HTTP calls between the WSO2 Integrator: MI and external clients, these logs are categorized using the HTTP call type.</li>
...
<td><div class="content-wrapper">
<p>This is a name that is generated to identify the connection between the WSO2 Integrator: MI and the external client (back-end or message sender).</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<td><div class="content-wrapper">
<p>The connection URL of the external client with which the message is being communicated. When reading a message, this corresponds to the URL the client invokes (i.e., the service or API URL in the WSO2 Integrator: MI). When writing a message to a backend service, it refers to the backend's URL.</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<li><strong>BACKEND LATENCY</strong> : The response message is received by the WSO2 Integrator: MI. This status corresponds to the time total time taken by the backend to process the message.</li>
