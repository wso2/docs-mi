...
When a client sends an HTTP request to the Micro Integrator, a Correlation ID can be included using the configured correlation header. By default, this header is set to `activityid`.

!!! note
    If the client does **not** include a Correlation ID in the request, the Micro Integrator will automatically generate one and assign it to the request.

The Correlation ID, whether provided by the client or generated by the Micro Integrator, is added to all log entries related to the same request. This helps you trace the complete flow of the request across the system.
...
<td><div class="content-wrapper">
<p>Each log contains a Correlation ID, which is unique to the HTTP request. A client can send the Correlation ID in the header of the HTTP request. If this Correlation ID is missing in the incoming request, the Micro Integrator will generate one for the request.</p>
<p>The name of the HTTP header used to carry the Correlation ID is configurable in the Micro Integrator.</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<ul>
<li><strong>HTTP</strong> call type identifies logs that correspond to either back-end latency or round-trip latency states. That is, in the case of an individual request, one log will be recorded to identify back-end latency, and another log for round-trip latency. Since these logs relate to HTTP calls between the Micro Integrator and external clients, these logs are categorized using the HTTP call type.</li>
...
<td><div class="content-wrapper">
<p>This is a name that is generated to identify the connection between the Micro Integrator and the external client (back-end or message sender).</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<td><div class="content-wrapper">
<p>The connection URL of the external client with which the message is being communicated. When reading a message, this corresponds to the URL the client invokes (i.e., the service or API URL in the Micro Integrator). When writing a message to a backend service, it refers to the backend's URL.</p>
<div class="code panel pdl" style="border-width: 1px;">
<div class="codeHeader panelHeader pdl" style="border-bottom-width: 1px;">
<strong>Example</strong>
</div>
...
<li><strong>BACKEND LATENCY</strong> : The response message is received by the Micro Integrator. This status corresponds to the time total time taken by the backend to process the message.</li>
