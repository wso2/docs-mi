...
detached) to an already running Java process. This universal agent uses the JVM agent API and is available for every Sun/Oracle JVM 11 and later.
1. Download [JVM-Agent](https://jolokia.org/download.html). (These instructions are tested with the Jolokia JVM-Agent version 2.6.0 by downloading the `jolokia-agent-jvm-2.6.0-javaagent.jar` file.)
