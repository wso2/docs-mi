...
## Filtering Mediator Spans and Adding Custom Span Tags

### Filtering Mediator Spans Globally

When tracing is enabled, every mediator in the message flow generates a span. This can result in traces cluttered with spans from mediators such as `LogMediator` or `PropertyMediator` that do not carry meaningful information. You can filter out spans of specific mediators using the `filtered.mediator.names` configuration.

Add the `filtered.mediator.names` property to the `[opentelemetry]` section in the `deployment.toml` file with a comma-separated list of mediator names to exclude from the trace:
...
Spans generated by the mediators listed in `filtered.mediator.names` will be excluded from the trace, reducing noise and making it easier to identify meaningful trace data. This configuration is global and applies to every integration deployed on the server.

The following mediator names are supported as filter values:
<details>
...
<table>
  ...
  <tbody>
    <tr><td><code>AggregateMediator</code></td></tr>
    <tr><td><code>CallMediator</code></td></tr>
    <tr><td><code>CalloutMediator</code></td></tr>
    <tr><td><code>CloneMediator</code></td></tr>
    <tr><td><code>DBLookupMediator</code></td></tr>
    <tr><td><code>DBReportMediator</code></td></tr>
    <tr><td><code>DropMediator</code></td></tr>
    <tr><td><code>EJBMediator</code></td></tr>
    <tr><td><code>EnrichMediator</code></td></tr>
    <tr><td><code>FaultMediator</code></td></tr>
    <tr><td><code>FilterMediator</code></td></tr>
    <tr><td><code>ForEachMediator</code></td></tr>
    <tr><td><code>ForEachMediatorV2</code></td></tr>
    <tr><td><code>HeaderMediator</code></td></tr>
    <tr><td><code>IterateMediator</code></td></tr>
    <tr><td><code>JSONTransformMediator</code></td></tr>
    <tr><td><code>LogMediator</code></td></tr>
    <tr><td><code>LoopBackMediator</code></td></tr>
    <tr><td><code>PayloadFactoryMediator</code></td></tr>
    <tr><td><code>PropertyGroupMediator</code></td></tr>
    <tr><td><code>PropertyMediator</code></td></tr>
    <tr><td><code>RespondMediator</code></td></tr>
    <tr><td><code>ScatterGather</code></td></tr>
    <tr><td><code>ScriptMediator</code></td></tr>
    <tr><td><code>SendMediator</code></td></tr>
    <tr><td><code>SequenceMediator</code></td></tr>
    <tr><td><code>SwitchMediator</code></td></tr>
    <tr><td><code>ThrottleMediator</code></td></tr>
    <tr><td><code>ThrowError</code></td></tr>
    <tr><td><code>TransactionMediator</code></td></tr>
    <tr><td><code>URLRewriteMediator</code></td></tr>
    <tr><td><code>ValidateMediator</code></td></tr>
    <tr><td><code>VariableMediator</code></td></tr>
    <tr><td><code>XQueryMediator</code></td></tr>
    <tr><td><code>XSLTMediator</code></td></tr>
  </tbody>
</table>
</details>
### Filtering Mediator Spans at the Mediator Level

In addition to the global `filtered.mediator.names` configuration, you can control span generation at the individual mediator level using the `traceFilter` attribute in the Synapse XML configuration. Setting `traceFilter="enable"` on a mediator will exclude its span in the trace. This provides fine-grained control over which mediator spans appear in the trace.

For example, to filter span from trace for a specific `PayloadFactory` mediator:

```xml
<payloadFactory traceFilter="enable" media-type="text" template-type="default">
    ...
</payloadFactory>
```

### Adding Custom HTTP Headers as Span Tags

You can propagate specific HTTP request headers as span tags by configuring the `custom.span.header.tags` property. This is useful when you want to correlate traces with application-level identifiers such as request or correlation IDs.

Add the `custom.span.header.tags` property to the `[opentelemetry]` section with a comma-separated list of header names:
...
type = "otlp"
url = "http://localhost:4317/v1/traces"
custom.span.header.tags = "X-Request-ID,X-Correlation-ID"
```

The headers listed in `custom.span.header.tags` will be extracted from incoming requests and added as tags to the corresponding spans. Multiple header names can be specified as a comma-separated string.
