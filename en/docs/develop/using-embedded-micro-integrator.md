# Using the Embedded WSO2 Integrator: MI

WSO2 Integration Studio contains an embedded WSO2 Integrator: MI instance, which you can use for testing your integration solutions during the development process.

## Deploy and run artifacts in the (embedded) server

Once you have the [integration artifacts packaged]({{base_path}}/develop/packaging-artifacts) in a composite application, you can deploy and run them in the embedded WSO2 Integrator: MI using a single click. 
...
3. Select <b>Run on WSO2 Integrator: MI</b> and click <b>OK</b>.
...
5.  Click **Finish**. The artifacts will be deployed in the WSO2 Integrator: MI and the server will start. 
...
Use the <b>Runtime Services</b> tab in WSO2 Integration Studio to view the endpoint URLs of the artifacts deployed in the embedded WSO2 Integrator: MI.

When you [deploy the artifacts and start](#deploy-and-run-artifacts-in-the-embedded-server) the embedded WSO2 Integrator: MI, the <b>Console</b> tab prints the server startup logs and the <b>Runtime Services</b> tab will open as shown below. 
...
1.  Open the [<b>Embedded WSO2 Integrator: MI Configuration</b>](#update-embedded-server-configs-and-libraries) dialog box. 
...
Hot deployment is enabled in the WSO2 Integrator: MI by default. This allows you to redeploy artifacts without restarting the server. However, if you have applied changes to the server configurations and libraries, the server will restart.
...
## Injecting environment variables to embedded WSO2 Integrator: MI

WSO2 Integrator: MI supports environment variables for server configurations as well as synapse configurations (integration artifacts).

!!!	Note
	To be able to dynamically inject parameters to the embedded WSO2 Integrator: MI, you must first define the relevant configurations as environment variables. See the following topics for instructions:
...
1.  [Deploy and run](#deploy-and-run-artifacts-in-the-embedded-server) the artifacts in the embedded WSO2 Integrator: MI.

    !!! Tip
        Note that you need to run the embedded WSO2 Integrator: MI at least once before proceeding to specify environment variables.
...
3.  In the **Run Configurations** dialog box that opens, select **WSO2 Integrator: MI Server 1.2.0** that is listed under **Generic Server** in the navigator:

    ![run configurations dialog box]({{base_path}}/assets/img/integrate/run-configs-dialog-box.png)

4.  In the **Server** tab, select WSO2 Integrator: MI 1.2.0 from the list if it is not already selected.
...
6.  Enter the variable name and value as a key-value pair and click **OK**. In this example, let's use the server offset:
    ...
    !!! Tip
        The offset parameter in the `deployment.toml` file of the embedded WSO2 Integrator: MI should be specified as follows:
        ```toml
        [server]
        offset="$env{offset}"
        ```
