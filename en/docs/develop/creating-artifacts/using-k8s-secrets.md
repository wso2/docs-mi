...
WSO2 Integrator: MI comes with a built-in secret repository as a part of its secure vault implementation by default. In addition to this, the WSO2 Integrator: MI also provides built-in support for Docker secrets and Kubernetes secrets for your containerized deployments.
...
The integration artifacts you develop using WSO2 Integrator: MI VS Code Extension are built into a Docker image from the Kubernetes Exporter module. Therefore, you must update the `integration_cr.yml` file (in your Kubernetes Exporter module) with the secrets you generated by using the following syntax:
