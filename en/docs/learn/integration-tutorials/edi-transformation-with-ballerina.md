...
3. Replace the generated sample code in the module file with the content from the `schema.bal` file you generated in [Step 3](#step-3-generate-ballerina-code-from-the-edi-schema). Then append the following MI wrapper functions at the end of the file:
    ...
    !!! note
        The functions `fromEdiString` and `toEdiString` are generated by the Ballerina EDI tool and will be present in the `schema.bal` content you copied.

4. Save the file, then click the **Build Ballerina Module** icon.
...
5. Once built, the module appears in the **Mediator palette** and its operations are available for use in integration flows.
