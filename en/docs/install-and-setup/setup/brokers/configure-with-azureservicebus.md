...
3. Add a shared access policy named `my-mq-policy` assigning **Send** and **Listen**. Go to **Home** > **my-mq** | **Queues** > **integration-queue (my-mq/integration-queue)** | **Shared access policies**.
    ...
    !!! Note
        The primary key will be generated by the system. If the primary key contains a + sign, encode it by replacing it with %2b in the jndi.properties file. This avoids issues in the Azure Service Bus library, which does not work correctly when the primary key contains the + sign.

        Alternatively, you can delete and create the policy again with the same name until you get a primary key without the + sign.
