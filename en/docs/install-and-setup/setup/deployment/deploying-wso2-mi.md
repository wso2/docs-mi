...
#### Coordination hardening for at-most-once task execution

The cluster can additionally guarantee that each scheduled occurrence of a coordinated task runs on **at most one node**, even through JVM pauses, coordination database stalls, failovers and redeployments. This is disabled by default and needs two extra tables and a rolling restart. See [Configure Coordination Hardening]({{base_path}}/install-and-setup/setup/feature-configs/configuring-coordination-hardening) for the setup, the readiness endpoint and the monitoring guide.
...
Create a simple Event Trigger using WSO2 Integrator: MI VS Code Extension and deploy it in the two WSO2 Integrator: MI servers. See the instructions on [creating a Scheduled trigger]({{base_path}}/develop/creating-artifacts/creating-scheduled-task).
