...
!!! Note
    ...
    if stricter access control is desired, you may restrict Management API write access to administrator users only by [configuring the make_non_admin_users_read_only]({{base_path}}/install-and-setup/setup/user-stores/managing-users#Restrict-non-admin-users-from-updating-the-deployed-artifacts) in the deployment settings 
